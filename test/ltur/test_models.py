...
class TestModels(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_journey = LturJourney(origin="Mars", destination="Earth",
    ...
    def test_journey_to_dict(self):
        ...
        self.assertEquals('Mars', actual_output['origin'])
        self.assertEquals('Earth', actual_output['destination'])
    ...
    def test_journey_to_string_utf8(self):
        self.test_journey.origin = u'Köln Hbf'
        expected_output = u"Köln Hbf to Earth on {dep} -> {arr} - 04:10 hrs - 2 changes " \
                          u"| 17.99 EUR (instead of 129.99 EUR)" \
            .format(dep=self.test_journey.departure.strftime(LturJourney.DATETIME_FORMAT),
                    arr=self.test_journey.arrival.strftime(LturJourney.DATETIME_FORMAT)
        )
